jQuery(document).ready(function () {
  ...
  jQuery.fn.setNav = function () {
    ...
    if (calScreenWidth >= 960) {
      ...
      jQuery('#menu-wrapper .nav li.menu-item').hover(
        function () {
          var documentWidth = parseInt(jQuery(document).width());
          var subMenu = jQuery(this).children('ul:first');
      ...
      jQuery('#menu-wrapper .nav li.menu-item')
    ...
    jQuery(window).resize(function () {
      jQuery('#menu-wrapper .nav li.menu-item ul.sub-menu').removeClass(
        'viewport-flip'
      );
    ...
    jQuery('body').on(
      ...
      function (event) {
        ...
        var menuContainerClass = jQuery(this)
          .parent('li')
          .parent('#mobile_main_menu.mobile-main-nav')
          .parent('div');

        var documentScroll = jQuery(document).scrollTop();
        var linkURL = jQuery(this).attr('href');
        var sectionPos = linkURL.indexOf('#');
        var sectionID = jQuery(this)
          .attr('href')
          .substr(sectionPos + 1);
        ...
        if (sectionPos > 0 && sectionID != '') {
          ...
          if (sectionID == 'top') {
            ...
            if (documentScroll != 0) {
              ...
              var scrollToPos = parseInt(
                jQuery('#' + sectionID).offset().top - topBarHeight + 82
              );
            }
        ...
        if (
          jQuery(this).parent('li').parent('ul').attr('id') ==
          'mobile_main_menu'
        ) {
        ...
        if (jQuerysublist.length > 0) {
          jQuery('#mobile_main_menu.mobile-main-nav').addClass(
            'mobile-nav-out'
          );
          jQuery('.mobile-menu-wrapper div #side-sub-menu').removeClass(
            'mobile-sub-nav-in'
          );
          jQuery('.mobile-menu-wrapper div #side-sub-menu').addClass(
            'mobile-nav-out'
          );

          if (jQuery('#pp_menu_layout').val() == 'full-burger-menu') {
            jQuery('.mobile-menu-wrapper .logo-container').fadeOut('slow');
            jQuery('.mobile-menu-wrapper .social-profile-wrapper').fadeOut(
              'slow'
            );
          }
          ...
          setTimeout(function () {
            ...
            var subMenuHTML =
              '<li><a href="javascript:;" id="mobile-back-btn" class="' +
              menuLevel +
              '" data-parent="' +
              parentMenu +
              '">' +
              startoPluginParams.backTitle +
              '</a></li>';
            subMenuHTML += jQuerysublist.html();

            menuContainerClass.append(
              '<ul id="side-sub-menu" class="nav ' + menuLevel + '"></ul>'
            );
            menuContainerClass.find('#side-sub-menu').html(subMenuHTML);
            menuContainerClass
              .find('#side-sub-menu')
              .addClass('mobile-sub-nav-in');
          }, 200);
        }
      }
    );
    ...
    jQuery('body').on('click', '#mobile-back-btn.top_level', function () {
      ...
      setTimeout(function () {
        ...
        if (
          jQuery('#mobile_main_menu.mobile-main-nav li#' + parentMenuId)
          ...
          parentMenu = jQuery(
            '#mobile_main_menu.mobile-main-nav li#' + parentMenuId
          )
            .parent('ul.sub-menu:first')
            .parent('li')
            .attr('id');
        ...
        subMenuHTML += jQuery(
          ...
          .parent('ul.sub-menu:first')
          .html();
        jQuery('.mobile-menu-wrapper div').append(
          '<ul id="side-sub-menu" class="nav ' + menuLevel + '"></ul>'
        );
